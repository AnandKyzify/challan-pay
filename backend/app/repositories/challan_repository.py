...
class ChallanDetailRepository:
    ...
    async def find_by_id(self, challan_id: str) -> dict[str, Any] | None:
        if not ObjectId.is_valid(challan_id):
            return None
        return await self.col.find_one({"_id": ObjectId(challan_id)})

    async def find_by_keys(self, challan_no: str, order_no: str) -> dict[str, Any] | None:
    ...
    async def mark_deleted(self, challan_id: str) -> dict[str, Any] | None:
        if not ObjectId.is_valid(challan_id):
            return None
